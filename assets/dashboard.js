(() => {
  ...
  function applyAppearance() {
    ...
    const vars={brand:a.brand,brandDark:a.brand_dark,sidebarBg:a.ink,sidebarText:a.nav_text};
  ...
  function editorMarkup() {
    ...
    return `<div class="tvcd-modal-backdrop"><div class="tvcd-modal"><div class="tvcd-modal-head"><div><small>${isNew?'Create':'Edit'}</small><h2>${esc(e.title||`New ${activeType().singular}`)}</h2></div><button class="tvcd-btn icon" data-close>${icon('no-alt')}</button></div><div class="tvcd-modal-body">
      <div class="tvcd-editor-layout"><div class="tvcd-editor-main">
        ${show('_post_title')?`<div class="tvcd-field tvcd-title-field"><label>Title</label><input data-core="title" value="${esc(e.title)}"></div>`:''}
      ...
      </div><aside class="tvcd-editor-sidebar">
        ${(show('_status')||show('_excerpt')||show('_featured_image'))?`<section class="tvcd-editor-panel"><h3>Post settings</h3>
          ${show('_status')?`<div class="tvcd-field"><label>Status</label><select data-core="status">${['draft','publish','pending','private'].map(s=>`<option ${s===e.status?'selected':''}>${s[0].toUpperCase()+s.slice(1)}</option>`).join('')}</select></div>`:''}
          ...
          ${show('_excerpt')?`<div class="tvcd-field"><label>Excerpt</label><textarea data-core="excerpt" rows="5">${esc(e.excerpt)}</textarea></div>`:''}
        </section>`:''}
      </aside></div>
