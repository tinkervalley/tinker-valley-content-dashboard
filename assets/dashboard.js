(() => {
  ...
  function render() {
    ...
    root.innerHTML = `<div class="tvcd-shell">
      <aside class="tvcd-sidebar ${state.navOpen?'open':''}">
        <div class="tvcd-brand">${state.appearance.light_logo_url||state.appearance.logo_url?`<img class="tvcd-brand-logo" src="${esc(state.appearance.light_logo_url||state.appearance.logo_url)}" alt="">`:'<div class="tvcd-mark">TV</div>'}<div><strong>Content</strong><small>${esc(boot.site.name)}</small></div><button class="tvcd-drawer-close" data-close-nav aria-label="Close navigation">${icon('no-alt')}</button></div>
        <nav class="tvcd-nav"><span class="tvcd-nav-label">Content</span>${state.types.filter(t=>t.enabled).map(t=>`<button data-type="${esc(t.name)}" class="${!state.settings&&!state.siteSettingsPage&&t.name===state.active?'active':''}"><i class="${esc(t.config.icon||'fa-solid fa-table-cells-large')}"></i><span>${esc(t.config.menu_label||t.label)}</span></button>`).join('')}${boot.canManage?`<div class="tvcd-nav-separator"></div><span class="tvcd-nav-label">Settings</span><button data-site-settings class="${state.siteSettingsPage?'active':''}"><i class="fa-solid fa-globe"></i><span>Site Settings</span></button><button data-settings class="${state.settings?'active':''}"><i class="fa-solid fa-gear"></i><span>Dashboard Settings</span></button>`:''}</nav>
    ...
    bind();
    requestAnimationFrame(alignFieldRows);
  }

  function applyAppearance() {
    const a=state.appearance||{};
    const vars={brand:a.brand,brandDark:a.brand_dark,ink:a.ink,paper:a.paper,header:a.header,navText:a.nav_text};
  ...
  async function saveSettings() {
    const payload={enabled_post_types:[],post_types:{},appearance:{...state.appearance}};
    ...
    try { await api('settings',{method:'POST',body:JSON.stringify(payload)}); const data=await api('bootstrap');state.types=data.postTypes;state.appearance=data.settings.appearance;state.active=state.types.find(t=>t.enabled)?.name||null;notify('Dashboard updated');render(); } catch(err){notify(err.message)}
  }
