...
= 0.9.5 =
* Refresh the dashboard with a Tailwind-inspired visual system.
* Standardize slate surfaces, spacing, radii, shadows, controls, and accessible focus rings.
* Preserve the configurable primary, secondary, sidebar background, and sidebar text palette.

= 0.9.4 =
* Widen the desktop content editor and add a dedicated sidebar for status, featured image, and excerpt.
* Simplify dashboard appearance settings to primary, secondary, sidebar background, and sidebar text colors.
* Restore custom palette support throughout the Notion-inspired navigation.
...
* Preserve HTML and block markup when saving post content.
* Automatically enable post content for existing editor configurations.
...
= 0.8.18 =
* Check for GitHub releases and install available plugin updates directly from Dashboard Settings.

= 0.8.17 =
* Add photos to gallery fields without replacing the existing selection.
* Remove individual gallery photos or clear the entire gallery.
* Reduce empty instruction spacing and tighten editor controls, tabs, buttons, and mobile layouts.

= 0.8.16 =
* Center the mobile navigation group when it fits and left-align it only when horizontal scrolling is required.

= 0.8.15 =
* Make mobile navigation buttons square and precisely center their icons.

= 0.8.14 =
* Decode WordPress HTML entities in titles, excerpts, and card descriptions before display.

= 0.8.13 =
* Display the WordPress featured image in the content editor.
* Save, replace, and remove featured images using the native WordPress thumbnail API.
* Add featured image to per-post-type editor field visibility settings.

= 0.8.12 =
* Redesign list view as a compact, continuous table-like surface with aligned rows and dividers.
* Add a responsive compact-row layout for list view on mobile.

= 0.8.11 =
* Display the regular dark logo in a polished, padded white navigation tile.
* Rename the navigation brand label from Content to Manage site.
